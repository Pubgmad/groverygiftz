...
async function seed() {
  ...
  const legacySampleProductSlugs = [
    'personalized-moon-lamp-frame',
    'premium-chocolate-gift-box',
    'love-couple-photo-frame-combo',
    'customized-racing-helmet-miniature',
    'elegant-gift-bouquet',
    'return-gift-pack-set-of-10',
    'personalized-led-name-lamp',
    'luxury-gift-hamper-gold-edition',
  ];
  const legacyDeleteResult = await db.collection('products').deleteMany({ slug: { $in: legacySampleProductSlugs } });
  console.log(`Legacy sample products removed: ${legacyDeleteResult.deletedCount}`);
